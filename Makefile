...
ps: $(PS_LIB) $(PS_MAIN)
app: build/ps
test: build/sample_test build/randomround_test
...
build/sample_test: src/test/sample_test.cc $(PS_LIB)
	$(CC) $(CFLAGS) $< $(PS_LIB) $(TESTFLAGS) -o $@
build/randomround_test: src/test/randomround_test.cc $(PS_LIB)
	$(CC) $(CFLAGS) $< $(PS_LIB) $(TESTFLAGS) -o $@
